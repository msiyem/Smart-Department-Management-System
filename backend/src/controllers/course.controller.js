...
export const createCourse = asyncHandler(async (req, res) => {
  ...
  return res.status(201).json(new ApiResponse(201, 'Course created.', { id: result.insertId }));
});
...
export const assignTeacher = asyncHandler(async (req, res) => {
  ...
  await pool.query('INSERT INTO course_teachers (teacher_id, course_id) VALUES (?, ?)', [teacher_id, course_id]);
  return res.status(201).json(new ApiResponse(201, 'Teacher assigned to course.'));
});
